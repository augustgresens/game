...
Actors::Actors(int cost_of_turn) : cost_of_turn{cost_of_turn} {}
...
bool Actors::take_turn(Engine& engine) {
    ...
    // if actor produced no action, then wait for them by not advancing to next
    // actor
    if (!action) {
        return false;
    }
    ...
    while (true) {
        ...
        if (result.next_action) {
            // if there is a subsequent action, set it as the next one to
            // perform
